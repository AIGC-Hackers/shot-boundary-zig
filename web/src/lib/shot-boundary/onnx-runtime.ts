...
export type ModelSource =
  ...
  | { kind: "bytes"; value: ArrayBuffer | Uint8Array }
...
export function configureWasmRuntime(options: WasmRuntimeOptions): void {
  if (options.wasmPaths !== undefined) {
    // Keep this as a runtime edge: each host app decides where static wasm assets live.
...
export function configureDefaultWasmRuntime(assetBaseUrl = "/ort-wasm/"): void {
  configureWasmRuntime(createWasmRuntimeOptions(assetBaseUrl))
}
  ...
  const session =
    source.kind === "file"
      ? await ort.InferenceSession.create(
          new Uint8Array(await source.value.arrayBuffer()),
          options
        )
      : source.kind === "bytes"
        ? await ort.InferenceSession.create(toUint8Array(source.value), options)
...
function readFloatOutput(
  ...
  if (output.data.length !== expectedLength) {
    throw new Error(
      `Model output '${name}' length does not match the requested batch.`
    )
  }
