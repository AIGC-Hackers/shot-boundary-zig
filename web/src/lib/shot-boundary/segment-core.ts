...
export function windowSourceIndices(frameCount: number): number[][] {
  ...
  for (
    let ptr = 0;
    ptr + modelSpec.windowFrames <= paddedCount;
    ptr += modelSpec.outputFramesPerWindow
  ) {
    const indices: number[] = []
    for (
      let paddedIndex = ptr;
      paddedIndex < ptr + modelSpec.windowFrames;
      paddedIndex += 1
    ) {
...
export function buildWindowBatch(
  ...
  const output = new Uint8Array(
    windows.length * modelSpec.windowFrames * bytesPerFrame
  )
  let cursor = 0
...
export function predictionsToScenes(
