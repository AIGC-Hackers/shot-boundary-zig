...
export async function decodeVideoToRgb24(
  ...
  try {
    ...
    const stats = await videoTrack.computePacketStats(
      Math.min(options.maxFrames, 100)
    )
    ...
    const measuredDurationSeconds =
      firstFrameTimestamp === null || frameCount <= 1
        ? 0
        : Math.max(0, lastFrameTimestamp - firstFrameTimestamp)
    const averageFps =
    ...
    const analyzedDurationSeconds =
      measuredDurationSeconds > 0
        ? measuredDurationSeconds + 1 / averageFps
        : frameCount / averageFps
...
export async function generateTimelineThumbnails(
  ...
  try {
    ...
    const usableDuration = Math.min(
      sourceDuration,
      durationLimitSeconds ?? sourceDuration
    )
    const sink = new CanvasSink(videoTrack, {
      width: 160,
      height: 90,
...
function evenlySpacedTimestamps(
  ...
  return Array.from(
    { length: count },
    (_, index) => startTimestamp + (lastOffset * index) / (count - 1)
  )
}
